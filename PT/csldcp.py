...
for seed in [2,144,145]:
    for shot in [1,4,8,16]:
        ...
        # 0.5253731343283582
        # prompt_template = ManualTemplate(text='{"placeholder":"text_a" }。我们一起好好思考一下，这包括{"mask"}。', tokenizer=tokenizer) # template2
        # 0.5308457711442786
        # prompt_template = ManualTemplate(text='{"placeholder":"text_a" }。真棒，让我们一块好好思考一下，这包括{"mask"}。', tokenizer=tokenizer) # template3
        # 0.5029850746268657
        # prompt_template = ManualTemplate(text='{"placeholder":"text_a" }。真棒，让我们一块认真思考一下，这包括{"mask"}。', tokenizer=tokenizer) # template3
        # prompt_template = ManualTemplate(text='好好思考一下，这包括{"mask"}：{"placeholder":"text_a" }。', tokenizer=tokenizer) # template3
